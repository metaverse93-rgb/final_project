"""
파인튜닝 후 평가 (qwen3.5-4b-finetuned-6ep)
testset_200.csv → 로컬 LoRA 어댑터 추론 → BLEU/COMET/TPR + G-Eval → results_200_finetuned_6ep.csv

실행:
    python eval/run_eval_finetuned_6ep.py                    # 전체 200건
    python eval/run_eval_finetuned_6ep.py --limit 10         # 빠른 테스트 (10건)
    python eval/run_eval_finetuned_6ep.py --skip-geval       # G-Eval 제외 (비용 절약)
    python eval/run_eval_finetuned_6ep.py --skip-comet       # COMET 제외 (느릴 때)
    python eval/run_eval_finetuned_6ep.py --skip-sheets      # Google Sheets 업로드 제외

결과:
    eval/data/results_200_finetuned_6ep.csv  (상세 결과, 베이스라인과 비교용)
    Google Sheets → "파인튜닝_6ep" 시트
"""
...
RESULTS_PATH   = os.path.join(os.path.dirname(__file__), "data", "results_200_finetuned_6ep.csv")
ADAPTER_PATH   = r"C:\Users\이동우\samseon\samsunmodel3.5_6ep"
...
MODEL_VERSION  = "qwen3.5-4b-finetuned-6ep"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="파인튜닝 후 평가 — 6ep (testset_200.csv)")
